...
export default function KbExplorer({ items }: { items: KbCardData[] }) {
  ...
  const mostRead = useMemo(
    () => [...items].sort((a, b) => b.views - a.views).slice(0, 4),
    [items],
  )
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10 lg:py-14">
      <div className="mb-10">
        <p className="text-sm font-semibold text-orange-500 mb-2">Vrijwilligersmatch Kennisbank</p>
        <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold tracking-tight text-gray-900 max-w-3xl">
          Handleidingen en how-to's voor gemeenten en organisaties
        </h1>
        <p className="mt-4 text-lg text-gray-600 max-w-2xl">
          Stap-voor-stap uitleg om Vrijwilligersmatch in te richten en te bedienen. Van white-label
          configuratie tot QR-check-in en SROI-rapportage.
        </p>
      </div>
          ...
          <div className="sticky top-16 z-10 bg-white/95 backdrop-blur-sm pb-4 mb-6 border-b border-gray-100">
            <div className="relative">
              ...
              <input
                ...
                placeholder="Zoek in de kennisbank op onderwerp of tag…"
              ...
              <div className="flex flex-wrap gap-2 mt-3">
                <button
                  ...
                  className={`px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${
                    activeTag === null ? "bg-orange-500 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
                  }`}
                >
                  Alles
                </button>
                {allTags.map((tag) => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-full text-xs font-medium transition-colors ${
                      activeTag === tag ? "bg-orange-500 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
                    }`}
                  >
                    {tag}
                  </button>
        ...
        <aside className="hidden lg:block">
          <div className="sticky top-24">
            ...
            <ol className="space-y-3">
              {mostRead.map((p, i) => (
                <li key={p.slug}>
                  <Link href={`/kennisbank/${p.slug}`} className="flex gap-3 group items-start">
                    <span className="text-lg font-bold text-orange-500/70 w-6 flex-shrink-0">{i + 1}</span>
            ...
            <div className="mt-8 p-5 bg-orange-50 rounded-2xl border border-orange-100">
              <h4 className="text-sm font-semibold text-gray-900 mb-1">Hulp nodig?</h4>
              <p className="text-xs text-gray-600 mb-3">
                Onze implementatiescripts en technische documentatie staan klaar voor gemeenten en organisaties.
              </p>
              <Link href="/vrijwilligerswerk" className="text-sm font-medium text-orange-600 hover:text-orange-700">
